...
def get_owner_access(request: Request):
    return templates.TemplateResponse(request, "owner/owner_access.html", {"active_page":"acceso"})
...
async def owner_registration(request: Request):
    return templates.TemplateResponse(request,"owner/form_owner.html", { "active_page":"registro" })
...
async def post_owner(request: Request,
    ...
    if(len(password) < 5):
           return templates.TemplateResponse(request,"owner/form_owner.html",
                                          { "active_page":"registro",
                                            "alert":"true",
                                            "alert_msg":"La clave debe de tener 5 caracteres minimo"
                                            })
